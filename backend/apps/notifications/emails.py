...
# --- 2. Stockist Approved ---
#
# Approval never emails a plaintext password. The live path
# (StockistViewSet.approve -> provision_stockist_login) sends a one-time
# set-password link via notify_stockist_approved_with_link (see #9 below), so
# a password is never generated by us or put on the wire. An older
# notify_stockist_approved() that mailed a plaintext credential was removed for
# that reason — do not reintroduce it.
...
def confirm_stockist_application(stockist_email: str, business_name: str, contact_name: str):
    ...
    body = (
        ...
        f"  2. If approved, you'll receive an email with a link to set your password\n"
        f"  3. Once you've set it, you can log in to browse our wholesale catalogue and place orders\n\n"

    ...
    <AdminLayout>
      <div className="max-w-5xl">
        <header className="mb-6">
          <h1 className="font-heading text-2xl md:text-3xl font-medium text-deep-blue">Getting started</h1>
          <p className="text-sm text-warm-gray-600 mt-2 leading-body">
        ...
        {/* In-page navigation */}
        <nav aria-label="Guide sections" className="mb-6">
          <ul className="flex flex-wrap gap-2">
            {GUIDE_SECTIONS.map((section) => (
              <li key={section.id}>
                <a
                  href={`#${section.id}`}
                  className="tap-target inline-flex items-center px-3 py-2 text-sm font-medium text-ocean bg-white border border-sand rounded-md hover:bg-ocean hover:text-white hover:border-ocean transition-colors focus:outline-none focus:ring-2 focus:ring-ocean"
                >
                  {section.label}
                </a>
              </li>
        ...
        <div className="space-y-6">
          {/* Setup checklist */}
          <section id="setup" className="bg-card-bg rounded-lg shadow-card p-5 sm:p-6 scroll-mt-6">
